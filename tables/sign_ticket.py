...
for sparsity in [0, .4, .5, .6, .7, .8, .9]:
    ...
    for task in ['MNLI', 'QQP', 'QNLI', 'SST-2', 'CoLA', 'MRPC', 'RTE']: # 'STS-B'
        ...
        for lr in ['2e-5','3e-5','4e-5','5e-5']:
            downstream_results = parse_file(f'models/{task}/gradual_prune_{int(sparsity*100)}_sign_ticket_lr_{lr}/eval_results.txt', EVAL_RESULTS_TEMPLATE)
